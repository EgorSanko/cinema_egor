...
// ⚠️ ВЫКЛЮЧЕНО снова (2026-07-30): при ПРЯМОЙ качке с VK сегменты начинают
// отдавать 403 на середине («Segment 161 failed too many times, skipping») →
// в файле дыры, иногда обрыв (тест на iOS дал 6.5с вместо фильма). У плеера
// этого нет, потому что бэкенд-прокси /api/alloha/seg делает 4 ретрая на
// сегмент. Правильный путь: вернуть качку через прокси (ради ретраев), но на
// ОТДЕЛЬНОМ httpx-клиенте, чтобы обрыв не убивал пул резолва (вариант Б).
