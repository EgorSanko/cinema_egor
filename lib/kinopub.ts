...
// 🚩 Кнопка «Скачать» (только PRO). ВКЛЮЧЕНА по варианту Б.
// Качаем через прокси /api/alloha/seg — у него 4 ретрая на сегмент (без них
// VK 403-ит сегменты на середине: дыры в файле, на iOS огрызок 6с). Прокси
// переведён на ОТДЕЛЬНЫЙ httpx-клиент (keepalive=0), поэтому обрыв качки
// больше не убивает пул резолва — раньше это гасило «Плеер 1» у всех.
