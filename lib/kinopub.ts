...
// 🚩 Кнопка «Скачать» (только PRO). Источник — kino.pub (Плеер 2, платный
// аккаунт): качается ровно, 30 мин контента без единой ошибки. Alloha НЕ
// годится — её ссылки отдают ~150 сегментов и дальше 403, фильм обрывался на
// 15-й минуте (не лечится ни ретраями, ни замедлением). HDRezka дала бы
// стабильные ссылки (живут месяцами), но её резолв банит наш IP.
export const DOWNLOADS_UP = true;
