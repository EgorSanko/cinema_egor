...
// 🚩 Кнопка «Скачать» ВЫКЛЮЧЕНА: Alloha не годится как источник для скачивания.
// Вариант Б (прокси с 4 ретраями + отдельный httpx-клиент) сделан и работает —
// изоляция пула ПОМОГЛА (резолв больше не падает от обрыва качки). Но сами
// ссылки Alloha/VK живут КОРОТКО: качка фильма идёт минуты, и к середине токены
// протухают → 403 на сегментах. Замеры: серия скачалась целиком (68 МБ), а
// следующая попытка фильма дала 0 байт; манифест то #EXTM3U, то HTML-403.
// Плееру это не мешает (он резолвит заново на каждую серию/запуск), а вот
// скачивание получается лотереей. Включать, когда вернётся HDRezka
// (HDREZKA_UP) или появится источник со стабильными ссылками.
