  ...
  return NextResponse.json(
    {
      ...
      // АВТОЗАПУСК обёртки при загрузке меню. Егор справедливо не понимал,
      // почему ввод старт-параметра открывает витрину MSX, а не приложение:
      // старт-параметр означает «загрузи меню», и обёртка была пунктом внутри.
      //
      // Сначала здесь стоял system:hbbtv:launch — Samsung отвечал «Current
      // platform does not support the action». Теперь link:, то самое действие,
      // которым в порталах открываются ВСЕ кинотеатры: в списке NoName у
      // Deeplex стоит link:http://smart.deeplex.cc, у Zona, VoKino, Filmix и
      // Lampa — точно так же. Меню остаётся под ним: вернувшись назад, человек
      // попадёт в каталог.
