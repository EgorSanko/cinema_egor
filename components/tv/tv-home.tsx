...
export function TvHome({ rails: serverRails }: { rails: TvRail[] }) {
  ...
  return (
    <main
      ...
      // ВЫСОТА ЭКРАНА, А НЕ «100% ОТ РОДИТЕЛЯ».
      //
      // Тут и была настоящая причина разрезанной шапки. height:100% работает
      // только если высота задана и у html, и у body — иначе он молча
      // превращается в auto: колонка не фиксируется, страница вырастает выше
      // экрана (в замере 2852 при кадре 1080) и прокручивается ЦЕЛИКОМ. При
      // переходе фокуса на нижние полки браузер уводит страницу вверх, шапка
      // уезжает за верхний край — её и разрезало пополам.
      //
      // 100vh считается от самого окна и ни от кого не зависит. Теперь
      // прокручивается только область карточек, как и задумано.
      style={{ background: "var(--background)", height: "100vh", overflow: "hidden" }}
    >
