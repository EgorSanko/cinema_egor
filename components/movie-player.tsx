...
import { getSource, resolveKinopub, resolveZenithEmbed, resolveIframeEmbed, isIframeSource, resolveAllohaHls, resolveVkMovie, resolveCdnHub, resolveRutube, resolveKinobd, pickAllohaStream, playerLabel, HDREZKA_UP, type AllohaHls } from "@/lib/kinopub";
...
export function MoviePlayer({ movie, variant }: MoviePlayerProps) {
  ...
  const resolveAllohaNative = useCallback(async (): Promise<boolean> => {
    ...
    if (getSource() === "vkmovie") {
      ...
      defQ = "1080p";
    } else if (getSource() === "cdnhub") {
      a = await resolveCdnHub(movie.id, "movie");
      defQ = "1080p";
    } else if (getSource() === "rutube") {
      const yr = movie.release_date ? new Date(movie.release_date).getFullYear() : "";
      a = await resolveRutube(movie.title || "", yr, (movie as any).original_title);
      defQ = "Авто";
    } else if (getSource() === "kinobd") {
      a = await resolveKinobd(movie.id, "movie");
      defQ = "1080";
    } else {
  ...
  useEffect(() => {
    ...
    const onSourceChange = () => {
      ...
      if ((getSource() === "alloha" || getSource() === "vkmovie" || getSource() === "cdnhub" || getSource() === "rutube" || getSource() === "kinobd")) {
        setStreamData(null);
        resolveAllohaNative();
  ...
  const fetchStream = async (translatorId?: number | null, _attempt = 0) => {
    ...
    if ((getSource() === "alloha" || getSource() === "vkmovie" || getSource() === "cdnhub" || getSource() === "rutube" || getSource() === "kinobd") && _attempt === 0) {
