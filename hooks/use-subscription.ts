...
export function useSubscription(): SubState {
  ...
  // 14.09.2026 — КИНОТЕАТР СТАЛ БЕСПЛАТНЫМ ДЛЯ ВСЕХ (решение Егора).
  //
  // Подписку никто не покупал: к этому дню осталась одна оплата через Tribute,
  // и тот человек сказал, что пользоваться не будет. Держать ради этого платный
  // контур бессмысленно, тем более что реклама всё равно есть — она встроена в
  // плеер Alloha, и убрать её мы не можем.
  //
  // Поэтому isPro теперь всегда true: выбор плееров, продолжить просмотр,
  // скачивание, «Вместе» и отсутствие нашего пре-ролла достаются каждому. Сам
  // расчёт подписки оставлен нетронутым — если решим вернуть платный тариф,
  // достаточно убрать эту заглушку.
  return {
    isPro: true,
