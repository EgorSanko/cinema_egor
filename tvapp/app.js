...
(function () {
  ...
  // Что этот телевизор реально умеет принять через MSE.
  //
  // Догадки тут не работают: «1080 не идёт» — это не про разрешение, а про
  // кодек раздачи. Спрашиваем один раз при запуске и шлём себе, чтобы дальше
  // выбирать поток по фактам, а не наугад.
  var КОДЕКИ = [
    ["H.264 базовый",  'video/mp4; codecs="avc1.42E01E"'],
    ["H.264 главный",  'video/mp4; codecs="avc1.4D401F"'],
    ["H.264 высокий",  'video/mp4; codecs="avc1.640028"'],
  ...
  var умеемЧерезMSE = {};
  function опроситьКодеки() {
    var части = [];
    for (var i = 0; i < КОДЕКИ.length; i++) {
      var да = false;
      try {
        да = !!(window.MediaSource && window.MediaSource.isTypeSupported(КОДЕКИ[i][1]));
      } catch (e) {}
      умеемЧерезMSE[КОДЕКИ[i][0]] = да;
      части.push(КОДЕКИ[i][0] + "=" + (да ? "да" : "НЕТ"));
    }
    маяк("кодеки MSE: " + части.join(", "));
  }

  // Маяки с ограничителем.
  //
  // Когда источник перестаёт отдавать видео, hls.js сыплет ошибками десятками
  // в секунду. Раньше каждая уезжала отдельной картинкой: за три минуты
  // телевизор отправил 1300 маяков, лог превратился в кашу, а сеть телевизора
  // была занята чем угодно, кроме видео. Теперь одинаковые сообщения копятся и
  // уходят одной строкой со счётчиком повторов.
  var _маякСост = { текст: "", когда: 0, повторов: 0, всего: 0 };
  function маяк(текст) {
    var сейчас = new Date().getTime();
    if (текст === _маякСост.текст && сейчас - _маякСост.когда < 10000) {
      _маякСост.повторов++;
      return;
    }
    var хвост = _маякСост.повторов > 0 ? " (прошлое повторилось " + _маякСост.повторов + " раз)" : "";
    _маякСост.текст = текст;
    _маякСост.когда = сейчас;
    _маякСост.повторов = 0;
    if (_маякСост.всего > 80) return;   // предел на один сеанс
    _маякСост.всего++;
    try { var i = new Image(); i.src = "/tv-error?m=" + encodeURIComponent(текст + хвост); } catch (e) {}
  }

  // 17.09.2026 — Про у всех (решение Егора): нашей рекламы перед фильмом нет
  // ни у кого, спрашивать сервер больше не нужно.
  ...
  function startPlayback(item, season, episode) {
    ...
    // 17.09.2026 (решение Егора): Alloha на телевизорах НЕ показываем — прямой
    // поток VK рвёт. Спрашиваем сразу Плееры 2–4 (cdnhub по коду IMDb, vkmovie
    ...
    tmdb("/" + type + "/" + item.id + "/external_ids", {}, function (ids) {
      ...
      if (nameUrl) {
        steps.push(nameUrl);
        steps.push(nameUrl.replace("/vkmovie?", "/rutube?"));
      }
    ...
    function первыйОтвет() {
      var осталось = steps.length;
      var отдали = false;
      for (var i = 0; i < steps.length; i++) {
        // get при таймауте может позвать ответ дважды (ontimeout и readyState 4) —
        // считаем каждый запрос ровно один раз, иначе «ни у кого нет» вылезет раньше.
        get(steps[i], однажды(function (d) {
          осталось--;
    ...
    function однажды(f) {
      var был = false;
      return function (d) { if (был) return; был = true; f(d); };
    }
  }
  ...
  function playTranslation(idx, useResume) {
    ...
    if (адрес.indexOf(".m3u8") >= 0 && window.Hls && window.Hls.isSupported()) {
      ...
      h.on(window.Hls.Events.ERROR, function (_e2, d2) {
        ...
        // СЕТЬ — САМОЕ ОПАСНОЕ МЕСТО.
        //
        // Здесь стоял голый startLoad(): он повторял мёртвый запрос без пауз и
        // без счёта. 12.09 ссылка на серию протухла на шестой минуте, и
        // телевизор за три минуты выпустил в источник больше двух тысяч
        // запросов. Источник счёл это нападением и закрыл сессию целиком
        // (X-VD: session_blocked) — после этого сериал не играл уже нигде и
        // ни у кого, пока блокировка не спала.
        //
        // Теперь: три мягкие попытки с нарастающей паузой, затем полный
        // перезапрос потока со свежей ссылкой с того же места, и лишь потом
        // сдаёмся. Источник больше не получает шторм.
        if (d2.type === window.Hls.ErrorTypes.NETWORK_ERROR) { сетьОтвалилась(); return; }

        function сетьОтвалилась() {
          var с = S.play.сеть || (S.play.сеть = { попыток: 0, свежих: 0, ждём: false });
          if (с.ждём) return;
          с.попыток++;
          if (с.попыток <= 3) {
            с.ждём = true;
            msg("Связь с источником прервалась, продолжаю…");
            setTimeout(function () {
              с.ждём = false;
              try { h.startLoad(); } catch (e6) {}
            }, с.попыток * 2000);
            return;
          }
          if (с.свежих < 2) {
            с.свежих++;
            с.попыток = 0;
            маяк("сеть: беру свежую ссылку, попытка " + с.свежих);
            msg("Обновляю ссылку на видео…");
            try { h.destroy(); } catch (e7) {}
            window.__hls = null;
            // false — продолжить с текущей секунды, а не с сохранённой позиции.
            setTimeout(function () { playTranslation(S.play.tr, false); }, 1500);
            return;
          }
          msg("Источник перестал отдавать это видео. Нажмите OK и выберите другую озвучку.");
        }
        ...
        // ПЕРВОЕ СПАСЕНИЕ — УРЕЗАТЬ БУФЕР.
        //
        // Самая частая причина фатальной ошибки на телевизоре — не кодек, а
        // переполнение квоты памяти под буфер. Тогда достаточно попросить
        // меньше и продолжить с того же места, не трогая ни качество, ни
        // озвучку. Человеку это видно как короткую заминку вместо «не
        // загрузилось».
        if (подробность === "bufferAppendError" && !S.play.буферУрезан) {
          S.play.буферУрезан = true;
          маяк("буфер: урезаю и пробую снова, качество:" + S.play.quality);
          msg("Подстраиваюсь под телевизор…");
          try { h.destroy(); } catch (e5) {}
          window.__hls = null;
          setTimeout(function () { playTranslation(S.play.tr, true); }, 300);
          return;
        }

        // ВТОРОЕ — ВСТРОЕННЫЙ ПЛЕЕР, как у Деплекса.
        //
        // Если и с малым буфером не пошло, дело уже не в памяти: телевизор не
        // принимает сам поток. Железный декодер берёт больше кодеков, чем MSE
        // — у Деплекса на этом же телевизоре играет 4K именно поэтому.
      ...
      h.on(window.Hls.Events.FRAG_BUFFERED, function () {
        // Пошло видео — прошлые сетевые неудачи больше не в счёт.
        if (S.play.сеть) S.play.сеть = { попыток: 0, свежих: S.play.сеть.свежих, ждём: false };
      });
    ...
    играемВстроенным(адрес, resumeAt, function () {
      // Не открылось — чаще всего канал не тянет выбранное качество. Спускаемся
      // на ступень ниже и пробуем снова: 720 вместо 1080 это вдвое меньше
      // данных. Сдаёмся только когда ступеней больше нет.
      var список = qualityList();
      var поз = -1;
      for (var z = 0; z < список.length; z++) {
        if (список[z] === S.play.quality) { поз = z; break; }
      ...
      S.play.попыток = (S.play.попыток || 0) + 1;
      if (поз >= 0 && поз + 1 < список.length && S.play.попыток <= 3) {
        S.play.quality = список[поз + 1];
        маяк("не открылось на " + список[поз] + ", пробую " + S.play.quality);
        msg("Качество " + список[поз] + " не пошло, пробую " + S.play.quality + "…");
        setTimeout(function () { playTranslation(S.play.tr, false); }, 400);
        return;
      }
  ...
  // Буфер меряем В БАЙТАХ, а не в секундах.
  //
  // Здесь была дорогая ошибка. Мы держали 45 секунд вперёд, и на 1080 при
  // 4.9 Мбит/с это ~28 МБ в памяти MSE. У телевизоров этого поколения квота
  // на буфер около 20 МБ: она переполнялась, браузер отказывался принять
  // кусок, и hls.js сообщал bufferAppendError — тот самый «1080 не грузится».
  // На 720 (2.9 Мбит/с) те же 45 секунд укладывались в квоту, поэтому 720
  // играл. Дело было не в кодеке и не в разрешении, а в памяти.
  //
  // Поэтому теперь главный ограничитель — maxBufferSize в байтах, а секунды
  // вторичны. Назад не держим ничего: отматывают редко, а память дороже.
  function настройкиБуфера(урезанный) {
    return {
      enableWorker: false,          // на слабых телевизорах отдельный поток только мешает
      maxBufferSize: урезанный ? 4 * 1000 * 1000 : 10 * 1000 * 1000,
      maxBufferLength: урезанный ? 6 : 15,
  ...
  function играемВстроенным(адрес, resumeAt, приОшибке) {
    var v = el("video");
    var сдался = false;
    маяк("встроенный: пробую, качество:" + S.play.quality);
    function неполучилось(почему) {
      if (сдался) return;
      сдался = true;
      var код = (v.error && v.error.code) || "?";
      маяк("встроенный: не взял (" + почему + ", код " + код +
           ", готовность " + v.readyState + ")");
      v.onerror = null;
    ...
    v.onerror = function () { неполучилось("ошибка видео"); };
    v.ontimeupdate = onTimeUpdate;
    v.onended = onEnded;
    // СКОЛЬКО ЖДАТЬ СТАРТА.
    //
    // Было девять секунд и один-единственный признак успеха — сдвинулось ли
    // время. На медленном канале этого мало: 12.09 у Егора кусок на 47 МБ шёл
    // 144 секунды, телевизор физически не успевал набрать буфер за девять
    // секунд, и мы трижды подряд решали «не взял» — поток вообще не открывался.
    //
    // Теперь ждём до тридцати секунд и считаем успехом ещё и готовность
    // телевизора: он уже принял достаточно данных, чтобы играть.
    var было = v.currentTime || 0;
    var прошло = 0;
    var часовой = setInterval(function () {
      if (сдался) { clearInterval(часовой); return; }
      прошло += 3;
      if ((v.currentTime || 0) > было + 0.2 || v.readyState >= 3) {
        сдался = true;
        clearInterval(часовой);
        маяк("встроенный: ПОШЛО, качество:" + S.play.quality + ", через " + прошло + " с");
        return;
      }
      // Совсем ничего не пришло за треть времени — ждать дальше бессмысленно.
      if (прошло >= 12 && v.readyState === 0 && !(v.buffered && v.buffered.length)) {
        clearInterval(часовой);
        неполучилось("за " + прошло + " с не пришло ничего");
        return;
      }
      if (прошло >= 30) {
        clearInterval(часовой);
        неполучилось("не запустилось за 30 с, готовность " + v.readyState);
      }
    }, 3000);
  }
  ...
  // СТОРОЖОК ЗАВИСАНИЯ.
  //
  // 12.09 у Егора картинка встала намертво: телевизор получил от нас ошибку и
  // один кусок, который шёл 47 секунд, после чего встроенный плеер перестал
  // запрашивать данные ВООБЩЕ — в логах ни одного запроса, и сам он уже не
  // оживал. Сторожок при запуске (9 секунд) такое не ловит: он смотрит только
  // на старт. Поэтому следим всё время: если видео должно играть, а секунды не
  // идут двадцать секунд подряд — перезапускаем поток с того же места.
  var сторожТаймер = null;
  function снятьСторожок() {
    if (сторожТаймер) { clearInterval(сторожТаймер); сторожТаймер = null; }
  }
  function завестиСторожок() {
    снятьСторожок();
    var последнее = -1, стоим = 0, спасений = 0;
    сторожТаймер = setInterval(function () {
      var v = el("video");
      if (!v || v.paused || v.ended || S.play.adActive) {
        стоим = 0;
        последнее = v ? (v.currentTime || 0) : -1;
        return;
      }
      var сейчас = v.currentTime || 0;
      if (Math.abs(сейчас - последнее) > 0.25) { последнее = сейчас; стоим = 0; return; }
      стоим += 5;
      if (стоим < 20) return;
      стоим = 0;
      спасений++;
      маяк("сторожок: картинка стоит на " + Math.round(сейчас) + " с, спасение " + спасений);
      if (спасений > 3) {
        msg("Видео не грузится. Нажмите OK и выберите другое качество или озвучку.");
        снятьСторожок();
        return;
      }
      msg("Связь подвисла, продолжаю…");
      // false — продолжить с текущей секунды, а не с сохранённой позиции.
      playTranslation(S.play.tr, false);
    }, 5000);
  }
