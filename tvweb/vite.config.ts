...
export default defineConfig({
  ...
  plugins: [
    ...
    legacy({
      ...
      // ТОЛЬКО совместимая сборка. Раньше отдавали обе, рассчитывая, что
      // телевизор возьмёт подходящую. Samsung на Tizen 5 брал ОБЕ: современную
      // не мог разобрать и падал («Unexpected token .» — это `?.`), а следом
      // запускал совместимую, и та шла в сеть ВТОРОЙ раз. В логах было десять
      // запросов вместо пяти, а на экране — вечная загрузка от умершей
      // половины. Замок в main.tsx от этого не спасал: падение происходило
      // раньше, чем он успевал сработать.
      // Оформление при этом не теряется — за него отвечает отдельный файл,
      // который выпускается независимо от набора скриптов.
      // Оформление Vite выпускает только вместе с современной сборкой — это
      // её известная особенность. Поэтому собираем в два прохода: первым
      // достаём файл оформления, вторым — сам скрипт. Управляет проходом
      // переменная TV_CSS_PASS, см. build.py рядом.
      renderModernChunks: !!process.env.TV_CSS_PASS,
