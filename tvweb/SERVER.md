...
## Ключ TMDB подставляет сервер

Раньше ключ лежал прямо в собранном скрипте: любой зритель мог прочитать его в
исходнике страницы, и проверка на секреты справедливо блокировала коммит.

Теперь приложение ходит на `/tmdb-api/` БЕЗ ключа, а nginx добавляет его сам:
...
location ^~ /tmdb-api/ { set $args $tmdb_args; proxy_pass https://api.themoviedb.org/3/; }
```

Карта объявлена ДО server-блока: внутри него `map` не разрешён. Старые версии
приложения, которые ключ ещё присылают, продолжают работать — если ключ в
запросе есть, подстановка не срабатывает.

Ключ живёт только в конфиге на сервере и в репозиторий не попадает.
