...
export function TvWatch({ media }: { media: TvWatchMedia }) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Под рекламой (FREE) ПЛЕЕРОМ не управляем: стрелки и OK глотает сам
      // пре-ролл, здесь гасим остальное; «Назад» — выход.
      //
      // ВАЖНО: только в плеере. Раньше проверки на экран не было, и у зрителя
      // без подписки эта строка съедала кнопки ВЕЗДЕ — в том числе на выборе
      // серии, где никакой рекламы нет. Подсветка не двигалась, «ОК» не
