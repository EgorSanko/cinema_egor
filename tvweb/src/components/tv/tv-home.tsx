...
export function TvHome({ rails: serverRails }: { rails: TvRail[] }) {
  ...
  // Облегчённого режима здесь БОЛЬШЕ НЕТ.
  //
  // Он придерживал картинки на шесть секунд и потом пускал по одной. Включался
  // по признаку «телевизор», а под него попадал и LG — там постеры стали
  // подгружаться отвратительно медленно, хотя LG всё тянет.
  //
  // Смысл в нём пропал: Samsung и подобные теперь уходят на лёгкий клиент
  // (развилка в index.html), а сюда попадают только те, кто справляется.
  const слабый = false;
  const картинокРазрешено = 9999;
