/** Android TV (APK → ТВ-страницы сайта): кнопка «Alloha»: видна, открывает окно, пульт управляет им, «назад» возвращает в наш плеер. */
...
(async () => {
  ...
  const out = {};
  await page.goto("https://sapkeflykino.ru/tv-login", { waitUntil: "domcontentloaded" });
  await page.evaluate(() => localStorage.setItem("user", JSON.stringify({ email: "tv-selftest@sapkeflykino.ru", name: "selftest" })));
  await page.goto("https://sapkeflykino.ru/tv-watch/movie/157336", { waitUntil: "domcontentloaded" });
