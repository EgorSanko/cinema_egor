...
(async () => {
  ...
  for (const режим of ["А: заранее cdnhub", "Б: переключение кнопкой"]) {
    const ctx = await b.newContext({ viewport: { width: 1280, height: 900 } });
    const p = await ctx.newPage();
    let проксиЗапросов = 0;
    p.on("request", (r) => { if (/cdnhub\/proxy/.test(r.url())) проксиЗапросов++; });
    ...
    await p.evaluate((зар) => {
      ...
      if (зар) localStorage.setItem("kino_source", "cdnhub");
    }, режим.startsWith("А"));
    await p.goto("https://sapkeflykino.ru/movie/9654", { waitUntil: "load" });
    await p.waitForTimeout(8000);
    if (режим.startsWith("Б")) { await жми(p, "Плеер 3"); await p.waitForTimeout(10000); }
    await жми(p, "watch");
