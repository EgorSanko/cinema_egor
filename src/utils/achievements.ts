...
// icon strings are slugs resolved by src/utils/achievement-icons.tsx to
// MaterialCommunityIcons names — gives consistent vector rendering across
// devices (vs emoji that look different on Android/iOS/MIUI).
export const ACHIEVEMENTS: AchievementDef[] = [
  { id: "first_movie", name: "Первый сеанс", desc: "Посмотри 1 фильм", icon: "clapperboard", category: "watch",
    rule: s => milestone(s.totalMoviesWatched, 1) },
  { id: "movies_10", name: "Киноман", desc: "Посмотри 10 фильмов", icon: "popcorn", category: "watch",
    rule: s => milestone(s.totalMoviesWatched, 10) },
  { id: "movies_50", name: "Кинокритик", desc: "Посмотри 50 фильмов", icon: "film", category: "watch",
    rule: s => milestone(s.totalMoviesWatched, 50) },
  { id: "movies_100", name: "Кинолегенда", desc: "Посмотри 100 фильмов", icon: "trophy", category: "watch",
    rule: s => milestone(s.totalMoviesWatched, 100) },

  { id: "first_episode", name: "Первая серия", desc: "Посмотри 1 серию", icon: "tv", category: "tv",
    rule: s => milestone(s.totalTvEpisodes, 1) },
  { id: "episodes_50", name: "Сериаломан", desc: "Посмотри 50 серий", icon: "sofa", category: "tv",
    rule: s => milestone(s.totalTvEpisodes, 50) },
  { id: "binge_5", name: "Бинж-мастер", desc: "5 серий за день", icon: "flame", category: "tv",
    rule: s => milestone(s.maxEpisodesInOneDay, 5) },
  { id: "binge_10", name: "Марафонец", desc: "10 серий за день", icon: "footprints", category: "tv",
    rule: s => milestone(s.maxEpisodesInOneDay, 10) },
  { id: "series_finished_1", name: "Сериал до конца", desc: "Досмотри сериал целиком", icon: "target", category: "tv",
    rule: s => milestone(s.completedTvSeries, 1) },

  { id: "night_owl", name: "Полуночник", desc: "Смотри после 02:00", icon: "moon", category: "time",
    rule: s => milestone(s.lateNightWatches, 1) },
  { id: "early_bird", name: "Жаворонок", desc: "Смотри утром (6-8)", icon: "bird", category: "time",
    rule: s => milestone(s.earlyMorningWatches, 1) },
  { id: "streak_7", name: "Завсегдатай", desc: "7 дней подряд", icon: "calendar-days", category: "time",
    rule: s => milestone(s.consecutiveDays, 7) },
  { id: "streak_30", name: "Преданный", desc: "30 дней подряд", icon: "crown", category: "time",
    rule: s => milestone(s.consecutiveDays, 30) },

  { id: "nostalgia", name: "Ностальгия", desc: "Фильм старше 1990 года", icon: "tape", category: "era",
    rule: s => milestone(s.oldFilmsWatched, 1) },
  { id: "retro", name: "Ретро-знаток", desc: "Фильм старше 1970 года", icon: "drama", category: "era",
    rule: s => milestone(s.retroFilmsWatched, 1) },
  { id: "premiere", name: "Премьера", desc: "Фильм этого года", icon: "sparkles", category: "era",
    rule: s => milestone(s.thisYearWatched, 1) },

  { id: "high_rated", name: "Высокий рейтинг", desc: "Фильм с оценкой 9.0+", icon: "star", category: "explore",
    rule: s => milestone(s.highRatedWatched, 1) },
  { id: "collector", name: "Коллекционер", desc: "25 в избранном", icon: "gem", category: "explore",
    rule: s => milestone(s.favoritesCount, 25) },
  { id: "archivist", name: "Архивариус", desc: "100 записей в истории", icon: "library", category: "explore",
    rule: s => milestone(s.totalMoviesWatched + s.totalTvEpisodes, 100) },
  { id: "polyglot", name: "Полиглот", desc: "5 разных озвучек", icon: "globe", category: "explore",
    rule: s => milestone(s.uniqueTranslators, 5) },

  { id: "hours_24", name: "Сутки в кино", desc: "24 часа просмотра", icon: "clock", category: "watch",
    rule: s => milestone(Math.floor(s.totalHoursWatched), 24) },
  { id: "hours_100", name: "Киноман-100", desc: "100 часов просмотра", icon: "hourglass", category: "watch",
